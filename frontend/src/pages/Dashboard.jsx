...
const Dashboard = (props) => {
  ...
  const getChats = async (emerge) => {
    try {
      const res = await fetch(`http://localhost:5000/api/v1/chats/${auth?.user?._id}/emerg/${emerge}`, {
        method: "GET",
        headers: { 'Content-type': "application/json" }
      });
      if (res.status === 200) {
                      ...
                      <div className="modal-body">
                        ...
                        <form >
                          <div className="d-flex">
                            <input className="form-control mx-3" value={txt} onChange={(e) => setTxt(e.target.value)} type="text" placeholder="enter your message"></input>
